...
def check_egfr(patient):
    ...
    out = {"id":"egfr_ckd_epi","units":"ml/min/1.73m2","timestamp":now_iso()}
    if egfr is None:
        out.update({"value": None, "status":"warning", "reason":"eGFR not available"})
        return out
    egfr = float(egfr)
...
def check_creatinine(patient):
    ...
    out = {"id":"creatinine_mg_dl","units":"mg/dL","timestamp":now_iso()}
    if cr is None:
        out.update({"value": None, "status":"warning", "reason":"Creatinine not available"})
        return out
    cr = float(cr)
...
def check_bun_ratio(patient):
    ...
    cr = patient.get("creatinine_mg_dl")
    out = {"id":"bun_creatinine_ratio","timestamp":now_iso()}
    if bun is None or cr is None:
        out.update({"value": None, "status":"warning", "reason":"Missing BUN or creatinine."})
        return out
...
def check_potassium(patient):
    k = patient.get("potassium_meq_l")
    out = {"id":"potassium_meq_l","units":"mEq/L","timestamp":now_iso()}
    if k is None:
        out.update({"value": None,"status":"warning","reason":"Potassium not available"})
        return out
    k = float(k)
...
def check_bmi_creatinine(patient):
    bmi = patient.get("bmi")
    cr = patient.get("creatinine_mg_dl")
    out = {"id":"bmi_creatinine_correlation","timestamp":now_iso()}
    if bmi is None or cr is None:
        out.update({"value":None,"status":"info","reason":"Missing BMI or creatinine."})
        return out
...
def check_lab_staleness(patient):
    ...
    out = {"id":"lab_staleness","timestamp": now_iso(), "value": {}}
    ...
    if not labs_ts:
        out.update({"status":"info","reason":"No lab timestamps provided."})
...
def run_renal_tool(patient: dict) -> dict:
    ...
    if "fail" in statuses:
        ...
        overall = "info"
    return {
        "tool_id":"renal_tool_v1",
        "checks":checks,
        "summary":{"overall":overall,"reasons":[c["reason"] for c in checks if c["status"]!="pass"],"confidence":0.92},
        "metadata":{"version":"1.0.0","run_ts":now_iso()}
    }
